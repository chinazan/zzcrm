...
class Record extends \Fox\Core\Services\Base
{
    ...
    public function findDeletedEntities($params)
    {
        $disableCount = false;
        if (in_array($this->entityType, $this->getConfig()->get('disabledCountQueryEntityList', array()))) {
            $disableCount = true;
        ...
        $collection = $this->getRepository()->find($selectParams, $deleted = true);
        ...
        if (!$disableCount) {
            $total = $this->getRepository()->count($selectParams, $deleted = true);
        } else {
            if ($maxSize && count($collection) > $maxSize) {
                $total = -1;
                unset($collection[count($collection) - 1]);
